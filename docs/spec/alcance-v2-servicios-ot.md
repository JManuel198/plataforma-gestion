...
- Pantalla de Clientes y Contactos — hoy es texto libre dentro de Servicio/OT. **Se decidió avanzar (2026-09-24):** grupo CRM del menú, Clientes en el Bloque 2 y Contactos en el Bloque 3; estado al 2026-09-25: Clientes ya tiene esquema (tabla `empresas`, ver su ficha en entidades.md) y pantalla completa en `/clientes` (listado, alta y edición con consulta de RUC por Decolecta, baja lógica), y Contactos ya tiene esquema (tabla `contactos` con FK a `empresas`, ver su ficha en entidades.md) y ya tiene su capa de servidor (`modules/contactos/`: actions, consultas y Zod) y el listado en `/contactos` (búsqueda, filtro de inactivos, baja lógica, vista de detalle y alta/edición en el modal, 2026-09-25). `orden_trabajo.cliente` sigue siendo texto libre: enlazarlo a `empresas` es un cambio aparte, no decidido. Sus reglas de negocio no están especificadas todavía: lo que no esté aquí va a preguntas-abiertas.md antes de construirse.
...
- Embudo de oportunidades (kanban) y flujo de aprobación. **Se decidió avanzar con el embudo (2026-09-24):** Bloque 4 del grupo CRM; estado al 2026-09-25: especificado en `oportunidades.md`, todavía sin tabla ni código (solo existe la ruta `/oportunidades` con su título). El flujo de aprobación no forma parte de esa decisión.
