...
| `filtros.ts` | El tipo `FiltrosServicios` (con `pagina`), `urlListado` y `contarFiltros` — la navegación por URL, sin Drizzle. |
| `queries.ts` | El listado paginado (`LIMIT`/`OFFSET`), con sus dos filtros combinados por AND, y `contarResultados` (el «de N» del pie; sin filtros, el total del contador). También documenta por qué este módulo no tiene `tipos.ts`. |
