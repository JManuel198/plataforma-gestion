...
type Props = {
  ...
  /** Servicio existente: se está viendo o editando. */
  servicio?: FilaServicio;
  /**
   * `created_at` ya formateada, para el modo "viendo". Viene de fuera porque se
   * resuelve en el servidor con la zona del negocio (ver `fila-servicio.tsx`).
...
/**
 * Alta, consulta y edición de un servicio sin salir del listado.
 *
 * Mismo patrón que `DialogoMaterial`, `DialogoListaPrecio`, `DialogoPersona` y
 * `DialogoOrdenTrabajo`, con las tres cosas que allí costó descubrir ya
 * incorporadas. No las quites sin leer por qué están:
 *
...
export function DialogoServicio({
  ...
  // `dialogo-material.tsx`.
  const nombre = servicio?.servicio?.trim() || "Sin descripción";
  const codigo = servicio?.codigo?.trim() || "Sin código";
  ...
  function alEnviar(formData: FormData) {
    iniciarGuardado(async () => {
      ...
      try {
        ...
        console.error(
          "[Servicios] fallo inesperado al guardar desde el modal",
          error,
        );
      ...
      toast.success(
        servicio ? `${nombre}: cambios guardados.` : "Servicio registrado.",
      );
      router.refresh();
    });
  }
