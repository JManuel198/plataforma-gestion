 ...
 * prefijo `use` — con el nombre en español, ESLint da error al ver `useRouter`
 * dentro. El archivo sigue en kebab-case como manda AGENTS.md. Mismo criterio
 * exacto que `modules/ordenes-trabajo/components/use-filtros.ts`, del que este
 * archivo es una copia salvo por el tipo de filtros.
 *
 ...
 * que hace que los dos se combinen en vez de pisarse. Pasar `undefined` en un
 * campo lo quita de la URL.
 *
 * El filtrado en sí ocurre en el servidor (queries.ts); estos componentes solo
 * navegan.
 */
export function useFiltros(filtros: FiltrosServicios) {
  const router = useRouter();
  const [navegando, iniciarNavegacion] = useTransition();

  function navegar(cambios: Partial<FiltrosServicios>, opciones: Opciones = {}) {
