...
import { CamposEmpresa } from "./campos-empresa";
import { VistaEmpresa } from "./vista-empresa";

type Props = {
  /**
   * Server Action que guarda. Devuelve el resultado, nunca redirige. Al
   * editar es `actualizarEmpresa` con el `id` ya fijado (`.bind`), así que
   * aquí las dos tienen la misma forma.
   */
  ...
  /** Empresa existente: se está viendo o editando. */
  empresa?: FilaEmpresa;
};

/**
 * Alta, consulta y edición de una empresa sin salir del listado.
 *
 * Mismo patrón que `DialogoPersona` y `DialogoMaterial`, con las tres cosas
 * que allí costó descubrir:
 *
 ...
 *    `router.refresh()` llegaría a campos todavía vivos. Además resetea el
 *    estado de `CamposEmpresa` (la consulta de RUC) entre una apertura y otra.
  ...
  function alEnviar(formData: FormData) {
    iniciarGuardado(async () => {
      ...
      try {
        ...
        console.error(
          "[Clientes] fallo inesperado al guardar desde el modal",
          error,
        );
      ...
      toast.success(
        empresa
          ? `${empresa.razon_social}: cambios guardados.`
          : "Empresa registrada.",
      );
      router.refresh();
    });
  ...
  return (
    ...
    >
      ...
      <DialogContent className="flex max-h-[85svh] flex-col data-closed:animate-none duration-0 sm:max-w-2xl">
        <DialogHeader>
          {empresa ? <ChipCodigo codigo={empresa.codigo} /> : null}
          <DialogTitle>
            {empresa
              ? editando
                ? "Editar empresa"
          ...
          <DialogDescription
            className={empresa && editando ? "sr-only" : undefined}
          >
            {empresa
              ? [
              ...
              : "Si tiene RUC, consúltalo para traer los datos de SUNAT."}
          </DialogDescription>
        </DialogHeader>
          ...
          >
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={enviando}>
                {enviando ? "Guardando…" : empresa ? "Guardar" : "Registrar"}
              </Button>
            </DialogFooter>
          </form>
        ) : (
