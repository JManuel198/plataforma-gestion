/**
 * Lista fija de países: código ISO 3166-1 alfa-2 y nombre en español.
 *
 * Es la lista del combobox de país (hoy, el formulario de Empresas) y la
 * fuente de verdad de qué códigos son válidos: el Zod del módulo valida contra
 * `CODIGOS_PAIS`, así que un código con forma correcta pero inexistente (`XX`)
 * se rechaza antes de tocar la base. El CHECK de la columna solo garantiza la
 * forma (dos mayúsculas).
 *
 * Vive en el módulo y NO en core/ a propósito: hoy solo la usa Clientes, y a
 * core/ sube lo que DOS módulos comparten (ver la deuda técnica de AGENTS.md,
 * MONEDAS vs. ESTADOS_OT). Cuando Contactos (Bloque 3) necesite país, ese es el
 * momento de moverla a core/ junto con `CampoPais` — nunca importarla desde
 * modules/contactos/.
 *
 * Se guarda el CÓDIGO, nunca el nombre (ver `empresas.pais` en
 * docs/spec/entidades.md): renombrar un país aquí no toca ninguna fila.
 *
 * Generado el 2026-09-25 con los 249 códigos oficiales de ISO 3166-1 y
 * `Intl.DisplayNames(["es"], { type: "region" })` de Node, ordenado
 * alfabéticamente en español. No importa nada: viaja al cliente sin arrastrar
 * nada más. Si hay que corregir un nombre, se edita aquí a mano.
 */
export const PAISES = [
...
] as const satisfies readonly { codigo: string; nombre: string }[];

export type Pais = (typeof PAISES)[number];
export type CodigoPais = Pais["codigo"];

/** Los códigos, para validar con `z.enum`. */
export const CODIGOS_PAIS = PAISES.map((pais) => pais.codigo) as [
  CodigoPais,
  ...CodigoPais[],
];

/** El país por defecto de una empresa nueva (decidido el 2026-09-25). */
export const PAIS_POR_DEFECTO: CodigoPais = "PE";

/** El nombre de un código, o el código tal cual si no está en la lista. */
export function nombrePais(codigo: string | null | undefined): string | null {
  if (!codigo) return null;
  return PAISES.find((pais) => pais.codigo === codigo)?.nombre ?? codigo;
}
